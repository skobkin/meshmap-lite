...
// InsertChatEvent appends a chat, system, or reaction event and returns its row ID.
...
INSERT INTO chat_events(event_type,channel_name,node_id,message_text,system_code,message_time,reported_at,observed_at,packet_id,mqtt_uploader_node_id,hop_start,hop_limit,reaction_emoji,reply_to_packet_id,created_at)
VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
`, string(e.EventType), e.ChannelName, nullIfEmpty(e.NodeID), messageText, nullIfEmpty(string(e.SystemCode)),
		e.MessageTime.UTC().Format(time.RFC3339Nano), ptrTime(e.ReportedAt), e.ObservedAt.UTC().Format(time.RFC3339Nano), ptrUint32(e.PacketID), nullIfEmpty(e.MQTTUploaderNodeID), ptrUint32(e.HopStart), ptrUint32(e.HopLimit), nullIfEmpty(e.ReactionEmoji), ptrUint32(e.ReplyToPacketID), e.CreatedAt.UTC().Format(time.RFC3339Nano))
	if err != nil {
		return 0, err
	}
...
SELECT e.id,e.event_type,e.channel_name,e.node_id,n.long_name,n.short_name,
       e.message_text,e.system_code,e.message_time,e.reported_at,e.observed_at,e.packet_id,e.mqtt_uploader_node_id,e.hop_start,e.hop_limit,e.reaction_emoji,e.reply_to_packet_id,mu.long_name,mu.short_name,e.created_at
FROM chat_events e
