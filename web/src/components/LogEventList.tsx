...
function getHopsBadge(event: LogEvent): { traversed: number; qualityClass: string; title: string; exhausted: boolean } | undefined {
  ...
  // Self-upload: the packet was generated by the MQTT gateway itself
  // (originator and uploader are the same node). The hop header in that
  // case is structurally meaningless — there was no LoRa transmission at
  // all between originator and uploader because they are the same radio —
  // so we don't surface a `↓0` badge. The chat view also never shows a
  // badge in this case (it already hides `↓0`).
  if (
